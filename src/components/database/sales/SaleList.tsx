      ...
      <table className="w-full">
        <thead className="bg-dark-750">
          <tr>
            ...
            <th className="px-4 py-2 text-left text-sm font-medium text-gray-400">Registro</th>
        ...
        <tbody>
          {sales.map((sale) => (
            <tr key={sale.id} className="border-t border-dark-700 hover:bg-dark-700/30">
              ...
              <td className="px-4 py-2 text-sm text-white">{sale.registro_venda}</td>
              <td className="px-4 py-2 text-sm text-gray-300">
                {(sale as any).cliente?.razao_social || sale.nome_cliente || '-'}
              </td>
              <td className="px-4 py-2 text-sm text-gray-300">
                {(sale as any).vendedor?.nome || '-'}
              </td>
              <td className="px-4 py-2 text-sm text-gray-300">
                {(sale as any).sdr?.nome || '-'}
              </td>
              <td className="px-4 py-2 text-sm text-gray-300">
                {(sale as any).servico?.nome}
              </td>
