...
const CategoryCompaniesModal: React.FC<CategoryCompaniesModalProps> = ({ isOpen, onClose, category }) => {
  ...
  const loadLinkedCompanies = async () => {
    return categoryService.getCategoryCompanies(category.id);
  };
  ...
  const handleLink = async () => {
    ...
    try {
      await categoryService.linkCategoryToCompany(category.id, selectedCompanyId);
      await loadData();
  ...
  const handleUnlink = async (empresaId: string) => {
    try {
      await categoryService.unlinkCategoryFromCompany(category.id, empresaId);
      await loadData();
    } catch (error) {
